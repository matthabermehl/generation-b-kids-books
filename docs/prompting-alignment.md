...
|---|---|---|---|
| Bitcoin stays clearly story-forward in caregiver or narrator framing before the ending without pitch energy | `buildBeatPlannerSystemPrompt`, `buildBeatPlannerPrompt`, `buildBeatRewritePrompt`, `buildPageWriterPrompt`, `buildCriticPrompt` | `runDeterministicBeatChecks` (`BITCOIN_THEME_INTEGRATION`, `BITCOIN_CHILD_LANGUAGE`) + policy-backed story checks in `validateBitcoinUsage` and `validateStoryTone` | `packages/prompts/test/beat-quality.test.ts` (`requires a high-salience Bitcoin beat before the protected ending window`) + `packages/domain/test/validators.test.ts` (`requires Bitcoin to appear before the final ending window...`) |
