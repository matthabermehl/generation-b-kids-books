...
- PR #6 is already merged to `master`; the character approval slice is live in repo state:
  - book-scoped `characterDescription` on order creation
  - `GET /v1/books/{bookId}/character`
  - `POST /v1/books/{bookId}/character/candidates`
  - `POST /v1/books/{bookId}/character/select`
  - checkout hard-blocked until `character_reference` exists
- Harness state:
  - `.agent/current_task.md` points at `scene-memory-01`
  - remaining failing tasks are `scene-memory-01`, `openai-image-provider-01`, and `review-render-image-cutover-01`
...
1. Scene memory contract
   - add `sceneId` and `sceneVisualDescription` to beats/pages/types/schemas
   - persist scene-aware page metadata in `story.json` and `pages.illustration_brief_json`
   - write current `scene_plan` and `image_plan` artifacts
   - cap `priorSameScenePageIds` at 2 and make them explicit in `image-plan.json`
2. Integrated `page_art` cutover
...
1. Branch for slice 1 from `master` and implement `scene-memory-01`.
2. Verify slice 1 with workers/api/smoke evidence, update harness state, push, and open PR to `master`.
3. Branch for slice 2 from slice 1 and implement the integrated `page_art` cutover.
4. Verify slice 2 with quality/live smoke evidence, update harness state, push, and open PR targeting the slice 1 branch so the PRs stay reviewable and stacked.
